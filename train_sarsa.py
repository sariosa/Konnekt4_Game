...
def train_sarsa_vs_random(
    ...
    """
    SARSA training as X vs random O.

    SARSA update uses the next chosen action a' (on-policy):
    ...
    for ep in range(episodes):
        ...
        # Only choose actions for the agent turn (X)
        while not done:
            ...
            # Choose a using current behavior policy (epsilon-greedy)
            ...
            # SARSA needs next action a' chosen under behavior policy at s'
            s_next = tuple(obs_after_o["board"]) + (int(obs_after_o["turn"]),)
            legal_next = info_after_o["legal columns"]
            ...
            if done:
                ...
                a_next = agent._get_action(env, obs_after_o)
                target = r_total + gamma * float(agent.Q[s_next][a_next])

            # Apply SARSA update manually (so we truly use Q(s',a') not max)
            agent.Q[s][a] = agent.Q[s][a] + alpha * (target - agent.Q[s][a])
